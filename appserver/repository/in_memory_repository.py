...
class InMemoryRepository(AbstractRepository, ABC):
    ...
    def view_games(self) -> List[GameEnviromentBase]:
        return [GameEnviromentBase(endpoint_name = game.endpoint_name, full_name = game.full_name) for game in self._games]

    def view_trainings(self, game_name: str) -> List[TrainingBase]:
        trainings = self._from_name(game_name).trainings
        return [TrainingBase(endpoint_name = training.endpoint_name, full_name = training.full_name) for training in trainings]

    def view_training(self, game_name: str, training_name: str) -> Training:
        return [training for training in self._from_name(game_name).trainings if training.endpoint_name == training_name]

    def create_training(self, game_name: str, training_base: TrainingBase) -> Tuple[bool, Training]:
        game = self._from_name(game_name)
        same_name_trainings = [training for training in game.trainings if training.endpoint_name == training_base.endpoint_name]
    ...
    def _get_full_training_configuration(self, game_name: str, training_name: str) -> \
        ...
        game = self._from_name(game_name)
        training_parameters = [training.parameters for training in game.trainings if training.endpoint_name == training_name][0]
        return (game.system_configuration, game.global_configuration, training_parameters)

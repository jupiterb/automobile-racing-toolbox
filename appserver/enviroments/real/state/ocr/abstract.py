...
class AbstractOcr:
    ...
    def _separated_digits(self, image: np.ndarray) -> list[np.ndarray]:
        ...
        rects = [(left, top, width, height) for left, top, width, height in rects \
        ...
        return [self.__normalize_shape(image[top:top+height, left:left+width])
    ...
    def __normalize_shape(self, image: np.ndarray) -> np.ndarray:
        if image.shape[1] < self._ocr_velocity_params.shape_width:
            image = cv2.copyMakeBorder(
                image,
                0, 0, self._ocr_velocity_params.shape_width - image.shape[1], 0, 
                cv2.BORDER_CONSTANT, 
                value=0
            )
        return cv2.resize(
            image,
            (self._ocr_velocity_params.shape_width, self._ocr_velocity_params.shape_height),
